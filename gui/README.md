...
这是 `serial-mcp-server` 的 Tauri 2 桌面端。它提供串口配置、共享连接列表、实时终端、MCP/CLI 活动视图、UTF-8/HEX 显示、UTF-8/HEX/Base64 发送、RTS/DTR 控制和发送输入框。
...
事件包含来源、客户端 PID、连接 ID、端口、方向、字节数、UTF-8 视图和 HEX 视图。设备 RX 由代理的单一后台读循环产生；GUI 不调用循环 `read`，因此不会消费掉 MCP/CLI 等待的数据。MCP 与 CLI reader 共享一个有界、消费式 RX FIFO；一方读走的字节不会再向另一方重放。
...
7. GUI 终端应实时出现对应事件：MCP 工具调用、TX 和控制线事件标记为 `MCP` 且 PID 为 MCP server 进程；物理 RX 标记为 `DEVICE` 且 PID 为 broker-owner 进程。TX/RX 内容可在 UTF-8 和 HEX 间切换。
8. 使用与步骤 1 完全相同的端口和参数运行 CLI。例如步骤 1 使用 COM6/19200、8N1、无流控时，可运行 `serial-mcp-server write --port COM6 --baud 19200 --data-bits 8 --stop-bits 1 --parity none --flow-control none --data PING --read --json`；GUI 应实时显示 `CLI` 调用与 TX、`DEVICE` RX，且设备不会被第二次打开。
9. 结束 MCP/CLI 进程，GUI 中的共享连接应继续存在并可操作。
10. 在 GUI 中显式关闭连接，MCP `list_connections` 中该连接应消失。

没有真实串口时，回到仓库根目录运行测试，验证跨进程代理发现、来源透传和事件日志：
...
代理默认绑定 loopback，但 `SERIAL_MCP_BROKER_ADDR` 覆盖值不会被强制限制为 loopback，且协议没有额外鉴权。不要把它配置到非本机接口。
